function dispiftrue(truefalse,str,inpath)
%DISPIFTRUE Displays str if truefalse is true
% if inpath is not specified, the caller path is given.

if truefalse
    
    if ~exist('inpath','var')
        callerpath=dbstack('-completenames');
        if length(callerpath)>1
            callerpath=callerpath(2);
        end
        inpaths=[callerpath.file,' line: ',num2str(callerpath.line)];
    else
        inpaths=inpath;
    end
    inpath=['  ( in ',inpaths,'  )  '];
...
%%%%%%%%%%%% massive function to print in colour:
function count = cprintf_fif(style,format,varargin)
